...
1. SDK提供的新的.Net程序集（MvCameraControl.Net.dll）
2. 库文件位于客户端安装路径下的 Development\DotNet\AnyCpu\netstandard2.0\MvCameraControl.Net.dll，基于 .Net Standard 2.0开发，同时提供了win32 和 win64版本
3. 如果需要基于低版本的.Net Framework框架开发，可以使用.Net Framework 4.0版本的库（位于Development\DotNet\AnyCpu\net40目录下）
4. 具体的调用方法可参考文档目录下的工业相机SDK开发指南(.Net)
...
namespace MvCameraControl:
1. It is a new .Net assembly (MvCameraControl.Net.dll) provided by the SDK
2. The library file is located in the Development\DotNet\AnyCPU\netstandard2.0\MvCameraControl.Net.dll folder under the client installation path, and is available in both win32 and win64 versions
3. If development based on a lower version of the .NET Framework is required, you can use the libraries from the .NET Framework 4.0 version (located in the Development\DotNet\AnyCpu\net40 directory).
